...
class CurrencyController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    if($data->currency == 'BDT') {
                        return '<span class="status active">Default Currency</span>';
                    }else {
    ...
    public function storeCurrency(CurrencyStoreRequest $request)
    {
        ...
        if(!empty($create)) {
            return redirect()->back()->with('toast_success', __('Currency Add successfully!'));
        }
    ...
    public function updateCurrency(Request $request, $id)
    {
        ...
        if(!is_null($currency)) {
            ...
            if(!empty($update)) {
                return redirect()->back()->with('toast_success', __('Currency Update successfully!'));
            }
        }
        return redirect()->back()->with('toast_error', __('Something went wrong!'));
    }
    ...
    public function deleteCurrency($id)
    {
        ...
        if(!empty($currency)) {
            return redirect()->back()->with('toast_success', __('Currency Deleted successfully!'));
        }
        return redirect()->back()->with('toast_error', __('Something went wrong!'));
    }
}
